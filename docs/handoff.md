...
- 当前图标视觉约束：
  ...
  - 黑色粗外轮廓
  - 黑色孔位
- 这轮图标微调的目标是尽量压缩左侧与底部留白；如果后续继续微调，优先只改 `src/app/icon.svg` 的 `transform` 位移参数，不要顺手改色或改指法。
- 本地外链分发成品当前保留在：
  - `exports/ocarina-c5-logo.png`
  - `exports/ocarina-c5-logo.jpg`
- 下面这些 2026-04-20 的内容仍然有效：

### 1.1.1 2026-04-20 历史补充
...
- 当前已确认一个新的 internal-only 方向：
  ...
  - 当前更推荐通过内部 HTML 集合页 + Playwright 导出实现，而不是简单图片拼接
  - 这条链当前仍属方向确认，尚未实现

### 1.1.2 2026-04-02 历史补充
...
- 截至 2026-04-19 当前工作区，这一轮最重要的新增公开内容是：
  - `yesterday`
